export const DestinationImagesAndVideoData = {
    Darjeeling: {
        subtitle: "Queen of Hills",
        description: "Enjoy the breathtaking views and cool climate of Darjeeling, the Queen of Hills, with its world-famous tea gardens and stunning mountain vistas.",
        hasVideo: false,
        hasPhotos: true,
        videos: [],
        photos: [
            {
                src: "https://images.unsplash.com/photo-1637737118663-f1a53ee1d5a7?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Scenic tea gardens and mountain views of Darjeeling"
            },
            {
                src: "https://images.unsplash.com/photo-1544894062-f500cf4fbd2c?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Beautiful landscapes and heritage sites"
            },
            {
                src: "https://images.unsplash.com/photo-1661970131022-714b905f7031?q=80&w=3132&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Panoramic views of the Himalayas"
            },
        ]
    },
    Goa: {
        subtitle: "Beach Paradise",
        description: "Discover the vibrant beaches and rich culture of Goa, where every moment is a celebration of life and adventure.",
        hasVideo: false,
        hasPhotos: true,
        videos: [],
        photos: [
            {
                src: "https://images.unsplash.com/photo-1496566084516-c5b96fcbd5c8?q=80&w=2072&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Pristine beaches and crystal clear waters"
            },
            {
                src: "https://images.unsplash.com/photo-1580741186862-c5d0bf2aff33?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Vibrant nightlife and cultural experiences"
            },
            {
                src: "https://images.unsplash.com/photo-1682743710558-b338ba285925?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Historic Portuguese architecture and churches"
            },
        ]
    },
    Jaipur: {
        subtitle: "Royal Heritage",
        description: "Explore the royal heritage and magnificent architecture of Jaipur, the Pink City of India, with its grand palaces and rich history.",
        hasVideo: false,
        hasPhotos: true,
        videos: [],
        photos: [
            {
                src: "https://prepseed.s3.ap-south-1.amazonaws.com/Jaipur.svg",
                content: "Iconic landmarks of the Pink City"
            },
            {
                src: "https://images.unsplash.com/photo-1539650116574-75c0c6d73c0e?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3",
                content: "Majestic palaces and royal architecture"
            },
            {
                src: "https://images.unsplash.com/photo-1578662996442-48f60103fc96?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3",
                content: "Rich cultural heritage and traditions"
            },
        ]
    },
    Kerala: {
        subtitle: "God's Own Country",
        description: "Experience the backwaters, lush greenery, and rich cultural heritage of God's Own Country, Kerala.",
        hasVideo: false,
        hasPhotos: true,
        videos: [],
        photos: [
            {
                src: "https://prepseed.s3.ap-south-1.amazonaws.com/Kerela.svg",
                content: "Serene backwaters and houseboat experiences"
            },
            {
                src: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3",
                content: "Lush green landscapes and tea plantations"
            },
            {
                src: "https://images.unsplash.com/photo-1469474968028-56623f02e42e?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3",
                content: "Beautiful beaches and traditional culture"
            },
        ]
    },
    Rishikesh: {
        subtitle: "Adventure Capital",
        description: "Experience the ultimate adventure in Rishikesh with thrilling activities and breathtaking landscapes that will leave you spellbound.",
        hasVideo: true,
        hasPhotos: false,
        videos: [
            {
                src: "https://s3.ap-south-1.amazonaws.com/prepseed/prod/ldoc/media/RishikeshVideo.mp4",
                content: "Thrilling adventure activities and scenic beauty of Rishikesh"
            }
        ],
        photos: []
    },
    Mysore: {
        subtitle: "Palace City",
        description: "Discover the grandeur of Mysore with its majestic palaces and rich cultural traditions.",
        hasVideo: false,
        hasPhotos: true,
        videos: [],
        photos: [
            {
                src: "https://plus.unsplash.com/premium_photo-1697730494992-7d5a0c46ea52?q=80&w=2071&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Magnificent Mysore Palace and royal heritage"
            },
            {
                src: "https://images.unsplash.com/photo-1698688513674-d38bea6a34c2?q=80&w=3133&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Rich cultural traditions and festivals"
            },
        ]
    },
    Matheran: {
        subtitle: "Hill Station Escape",
        description: "Escape to the serene hills of Matheran and immerse yourself in nature's tranquility and scenic beauty.",
        hasVideo: false,
        hasPhotos: true,
        videos: [],
        photos: [
            {
                src: "https://images.unsplash.com/photo-1663070549709-8b524a0560e7?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Peaceful hill station with panoramic views"
            },
            {
                src: "https://images.unsplash.com/photo-1590812854696-65cefa66f181?q=80&w=2108&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
                content: "Nature trails and scenic viewpoints"
            },
        ]
    },
    Saputara: {
        subtitle: "Hill Station Gem",
        description: "Experience the natural beauty and peaceful atmosphere of Saputara hill station.",
        hasVideo: false,
        hasPhotos: true,
        videos: [],
        photos: [
            {
                src: "https://s3.ap-south-1.amazonaws.com/prepseed/prod/ldoc/media/SaputaraHillStationImage1.jpg",
                content: "Beautiful hill station with lush greenery"
            },
            {
                src: "https://s3.ap-south-1.amazonaws.com/prepseed/prod/ldoc/media/SaputaraHillStationImage2.jpg",
                content: "Scenic landscapes and peaceful environment"
            },
        ]
    },
    Mandvi: {
        subtitle: "Beach Paradise",
        description: "Discover the pristine beaches and serene atmosphere of Mandvi.",
        hasVideo: false,
        hasPhotos: true,
        videos: [],
        photos: [
            {
                src: "https://prepseed.s3.ap-south-1.amazonaws.com/Mandvi+Beach+(1).jpg",
                content: "Beautiful beaches and coastal views"
            },
            {
                src: "https://prepseed.s3.ap-south-1.amazonaws.com/Mandvi+Beach+(2).jpg",
                content: "Serene coastal atmosphere"
            },
            {
                src: "https://prepseed.s3.ap-south-1.amazonaws.com/mandvi(3).jpg",
                content: "Pristine beach experiences"
            },
        ]
    },
    "Shivrajpur Beach": {
        subtitle: "Beach Paradise",
        description: "Experience the beautiful beaches and coastal beauty of Shivrajpur.",
        hasVideo: false,
        hasPhotos: true,
        videos: [],
        photos: [
            {
                src: "https://prepseed.s3.ap-south-1.amazonaws.com/shivrajpur+(1).jpeg",
                content: "Pristine beach views"
            },
            {
                src: "https://prepseed.s3.ap-south-1.amazonaws.com/shivrajpur+(2).jpeg",
                content: "Beautiful coastal landscapes"
            },
            {
                src: "https://prepseed.s3.ap-south-1.amazonaws.com/shivrajpur+(3).jpg",
                content: "Serene beach experiences"
            },
        ]
    },
}

